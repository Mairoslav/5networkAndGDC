...
struct House: Codable {
    ...
    // 02:20 And the format is "enum CodingKeys:"... see the code and comments above inserted within struct House., after the colon, the raw value is of type "String". And to make this work, we also need to make sure this enum conformts to the CodingKey protocol
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case hasAirConditioning = "has air conditioning" // 04:08 with spaces between each word, just like the JSON is formatted (Why not _ as per JSON?)
    }
}
    ...
    // 04:20 So we have maped the JSON keys to our own Swift property names. There's just one more thing I'd like to show you about CodingKeys before you try it out on your own. Remember the location case? Both our Swift property and our JSON key are called location. Did we really need to add this case to our CodingKeys enum? Let's remove it to find out (commented out***). Error:
/*
expression failed to parse:
error: 5. Coding Keys.xcplaygroundpage:25:8: error: type 'House' does not conform to protocol 'Decodable'
struct House: Codable {
       ^
...
5. Coding Keys.xcplaygroundpage:28:9: note: cannot automatically synthesize 'Decodable' because 'location' does not have a matching CodingKey and does not have a default value
    let location: String
 */
...
// 05:03 Let's look at the second(actually 3rd) error message, location does not have a matching CodingKey and does not have a default value. So that sounds like this error occurrs because the location key is missing. Even though its property name is the same as the JSON key, when we specify a coding keys' enum, we override the dafault behaviour in which Swift detects the JSON keys. If we set a cas for one key, we need to set a case for all the keys. Like our error states, we ould provide a default value for location. For example, make it an empty String ******.
// 05:39 That's the error, but the location is not actually parsed. If you want to get this value fro the JSON, we need to specify a case. That's all for now on CodingKeys. Not it's your turn. I provided an interactive example, so that you  can try your hand at using CodingKeys. Simply mofifiy the struct so that it maps the JSON keys to the struct cusom property names.
...
// 00:47 First, it (error) says: "key not found" (at my side shows "dataCorrupted - see below whole error message"). Remember that keys are the equivalent of property names in JSON object. So, looks like the property houseType, from our struct House, is not present.

// 01:00 And the next line, debugDescription: "no value associated with key CodingKeys" (at my side shows: "NSDebugDescription=Badly formed object")

/// error shown at my side: "dataCorrupted(Swift.DecodingError.Context(codingPath: [], debugDescription: "The given data was not valid JSON.", underlyingError: Optional(Error Domain=NSCocoaErrorDomain Code=3840 "Badly formed object around line 3, column 4." UserInfo={NSDebugDescription=Badly formed object around line 3, column 4., NSJSONSerializationErrorIndex=29})))"
