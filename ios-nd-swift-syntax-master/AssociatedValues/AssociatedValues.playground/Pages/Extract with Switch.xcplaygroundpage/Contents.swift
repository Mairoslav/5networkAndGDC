...
case .sepia:
    print("sepia")
// case let .verticalGradient(from: color1, to: color2): // alternative
case .verticalGradient(let color1, let color2):
    print("vertical gradient with \(color1) and \(color2) £££££££££")
case let .horizontalGradient(from: color1, to: color2):
...
// 2:55 If you want to treat two cases with the same associated value type the same way, you can specify them both in a simple case statement. Here in 2nd case we treat the vertical and horizontal gradient in the same way. We jus separate these two particular cases with comma.
switch filter1 {
case .sepia:
    print("sepia")
case .verticalGradient(let color1, let color2), // instead of : use ,
    // print("vertical gradient with \(color1) and \(color2) £££££££££")
    .horizontalGradient(let color1, let color2): // delete case
    print("horizontal gradient with \(color1) and \(color2) @@@@@@@")
case .sketch(let penThickness):
    if let thickness = penThickness {
        print("sketch using \(thickness) thickness")
